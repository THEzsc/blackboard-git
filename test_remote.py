...
class RemoteTest(unittest.TestCase):
    ...
    def test_real_git_clone_and_pull(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (source/'folder.html').write_text('folder', encoding='utf-8');(source/'file.html').write_text('file', encoding='utf-8')
            ...
            self.assertEqual('first',(dest/'Content/Notes.txt').read_text(encoding='utf-8'))
            ...
            self.assertEqual('first',(second/'Content/Notes.txt').read_text(encoding='utf-8'))
            first=command('-C',str(dest),'rev-parse','HEAD').stdout
            version('second');command('-C',str(dest),'pull','--ff-only')
            self.assertEqual('second',(dest/'Content/Notes.txt').read_text(encoding='utf-8'))
            ...
            (dest/'Content/Notes.txt').write_text('local edit', encoding='utf-8');version('third')
            self.assertNotEqual(0,command('-C',str(dest),'pull','--ff-only',check=False).returncode)
            self.assertEqual('local edit',(dest/'Content/Notes.txt').read_text(encoding='utf-8'))
